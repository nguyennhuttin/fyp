...
def train(config):
    ...
    try:
        ...
        print('saved entire train_loader')

    except:
        torch.save(next(iter(train_dataloader)), 'train_loader.pt')
        print('save 1 iter from train_loader')
    ...
    for epoch_idx in tqdm(range(config.train.get('epochs', 10))):
        ...
        for batch_idx, batch in enumerate(train_dataloader):
            ...
            print([x.shape for x in batch['audio']])
            ...
            if batch_idx % config.wandb.get('log_interval', 5000) == 0:
                ...
                print('\ntarget,output: ', list(
                    zip(target_strings, decoded_output)))
                wandb.log({
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', default='quartznet-pytorch/configs/train_LJSpeech.yaml',
